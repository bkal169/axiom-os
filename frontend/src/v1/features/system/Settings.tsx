...
const TeamSection = ({ tier }: { tier: string }) => (
    <Card title="Team Management" action={<Badge label={tier.toUpperCase()} color="var(--c-gold)" />}>
        ...
        <div className="axiom-widget-placeholder">
            <div className="axiom-text-14-dim" style={{ marginBottom: 10 }}>Team management coming soon to V1</div>
        ...
        <Tabs tabs={["Profile", "API Keys", "Notifications", "Team", "Connection"]}>
            <div>
                <Card title="User Profile">
                    <div className="axiom-grid-3" style={{ gap: 14 }}>
                        <Field label="Full Name"><input className="axiom-input" value={profile.name} onChange={pu("name")} placeholder="Your name" /></Field>
                        <Field label="Email"><input className="axiom-input" value={profile.email} onChange={pu("email")} placeholder="email@company.com" /></Field>
            ...
            <div>
                <Card title="API Key Management">
                    <div className="axiom-text-12-dim" style={{ marginBottom: 14 }}>
                        Configure your LLM proxy for production, or use direct API keys for development.
                    </div>

                    <div className="axiom-card-inner-14" style={{ marginBottom: 14 }}>
                        ...
                        <div style={{ fontSize: 10, color: apiKeys.proxyUrl ? "var(--c-green)" : "var(--c-amber)", marginTop: 4 }}>
                            {apiKeys.proxyUrl ? "✓ Proxy configured — API keys are kept server-side" : "⚠ No proxy — keys are exposed in the browser (dev mode only)"}
                        </div>
                    </div>

                    <div className="axiom-grid-2" style={{ gap: 14 }}>
                        {[
                ...
                <Card title="Notification Preferences">
                    {[
                        ...
                        <div key={key} className="axiom-flex-center-gap-12-p10-bb">
                            <input type="checkbox" checked={!!(notifs as any)[key]} onChange={() => setNotifs({ ...notifs, [key]: !(notifs as any)[key] })} style={{ accentColor: "var(--c-gold)", width: 14, height: 14 }} />
            ...
            <div>
                <Card title="Supabase Connection" action={<Badge label={supa.configured() ? "Connected" : "Not Set"} color={supa.configured() ? "var(--c-green)" : "var(--c-dim)"} />}>
                    <div className="axiom-text-11-dim" style={{ marginBottom: 12 }}>
                        Connect to Supabase for cloud persistence, multi-device sync, and team collaboration.
                    </div>
                    <div className="axiom-grid-2" style={{ gap: 14 }}>
                        <Field label="Supabase URL">
                            <input className="axiom-input" value={localStorage.getItem("axiom_supa_url") || ""} onChange={e => localStorage.setItem("axiom_supa_url", e.target.value)} placeholder="https://xxxxx.supabase.co" />
                        </Field>
                        <Field label="Anon / Publishable Key">
                            <input className="axiom-input" type="password" value={localStorage.getItem("axiom_supa_key") || ""} onChange={e => localStorage.setItem("axiom_supa_key", e.target.value)} placeholder="eyJhbGci..." />
                        </Field>
                    </div>
                    <div className="axiom-flex-gap-8" style={{ marginTop: 16 }}>
                ...
                <Card title="Data Management">
                    {[
                        ...
                        <div key={i} className="axiom-flex-center-gap-12-p10-bb">
                            <div className="axiom-flex-1">
                                <div style={{ fontSize: 13, color: i >= 1 ? "var(--c-red)" : "var(--c-text)" }}>{l}</div>
